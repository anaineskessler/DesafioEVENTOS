let listado_pedido =document.getElementById("listado_pedido");
...
function verpedidofinal(listado){
    for (const lista of listado) {
        let item = document.createElement("div");
        item.innerHTML = `<h3>Producto: ${lista.producto}</h3> 
                        <p>Cantidad: ${lista.cantidad} \n Costo: $${lista.costo}</p>`;
        listado_pedido.append(item);
    }
}
...
totalpedido = calculototal();
let pedidofinal = document.createElement("div");
if (promocion != "" & total > 0){
    // pedido=verpedido(pedidoproducto);
    pedidofinal.innerHTML =`<h3>Valor del pedido con la promoción ${promocion} es <b>$ ${total}</b></h3> `;
    verpedidofinal(pedidoproducto);
    total_pedido.append(pedidofinal);
    // alert(pedido+"\n"+pedidofinal);
} else if (totalpedido > 0) {
    // pedido=verpedido(pedidoproducto);
    pedidofinal.innerHTML =`<h3>Valor del pedido es <b>$ ${totalpedido}</b></h3> `;
    verpedidofinal(pedidoproducto);
    total_pedido.append(pedidofinal);
        //alert(pedido+"\n"+pedidofinal);
} else {
